...
class Worker extends EventEmitter {
  ...
  postMessage(payload, transferList) {
    this[kPublicPort].postMessage(payload, transferList);
  }
...
function setupChild(evalScript) {
  ...
  publicWorker.postMessage = (payload, transferList) => {
    publicPort.postMessage(payload, transferList);
  };

  port.on('message', (message) => {
    if (message.type === 'loadScript') {
      const { filename, doEval, workerData, hasStdin } = message;
      publicPort = message.publicPort;
      publicPort.on('message', (payload) => {
        publicWorker.emit('workerMessage', payload);
      });
      publicWorker.workerData = workerData;
      setupPortReferencing(publicPort, publicWorker, 'workerMessage');
